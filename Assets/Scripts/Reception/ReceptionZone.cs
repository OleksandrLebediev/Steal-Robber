...
public class ReceptionZone : MonoBehaviour
{
    ...
    private IEnumerator CollectObjectAnimation(IObjectForCollect objectForCollect, ISender sender)
    {
        ...
        objectForCollect.CurrentTransform.localPosition = new Vector3(target.x, target.y, target.z);                                                                                                                       // itemForCollect.CurrentTransform.localRotation = Quaternion.AngleAxis(30, Vector3.up); // Óñòàíîâêà êîí÷íûõ ðåóëüòàòîâ äëÿ ïîâîðîòà
